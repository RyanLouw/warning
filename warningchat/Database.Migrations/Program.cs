...
public class Program
{
    ...
    private static IServiceProvider CreateServices(MigratorTag migratorTag)
    {
        ...
        // Aspire injects database references as ConnectionStrings__{resourceName}.
        // Read that value directly so a connection string from central or JSON
        // configuration cannot replace the connection generated by AppHost.
        var aspireConnectionKey = $"ConnectionStrings__{migratorTag.ConnectionKey}";
        var aspireConnection = Environment.GetEnvironmentVariable(aspireConnectionKey);
        var connection = !string.IsNullOrWhiteSpace(aspireConnection)
            ? aspireConnection
            : builder.Configuration.GetConnectionString(migratorTag.ConnectionKey);
        ...
        Log.Information(
            "Using {ConnectionSource} connection string for {ConnectionKey}",
            !string.IsNullOrWhiteSpace(aspireConnection) ? "Aspire" : "host configuration",
            migratorTag.ConnectionKey);
